...
IMG=opensourceway/omni-repositiory:$(GIT_COMMIT)
ARCHITECTURE ?= "amd64"
...
manager: fmt vet swagger-doc
	go build -ldflags "-X main.Tag=$(GIT_TAG) -X main.CommitID=$(GIT_COMMIT) -X main.ReleaseAt=$(RELEASED_AT)"  -o bin/omni-repositiory main.go

# Run up server
# NOTE(tommylike): do not use go run command for the issue of signal relay issue
run: manager
	./bin/omni-repositiory run
